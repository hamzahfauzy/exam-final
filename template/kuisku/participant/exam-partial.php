        ...
        <div class="col-sm-12">
            ...
            <form method="post">
                ...
                <div class="finish-section container">
                    <?php if($no != 1): ?>
                    <a href="<?= route('participant/exam-partial/'.($no-1)) ?>?token=<?=$_GET['token']?>" data-page="<?=$no-1?>" class="btn btn-success" onclick="prevQuestion(this)"><i class="fa fa-arrow-left fa-fw"></i> Sebelumnya</a>
                    <?php endif ?>
                    ...
                    <?php if($no != $numOf): ?>
                    <a href="<?= route('participant/exam-partial/'.($no+1)) ?>?token=<?=$_GET['token']?>" data-page="<?=$no+1?>" class="btn btn-success" onclick="nextQuestion(this)"><i class="fa fa-arrow-right fa-fw"></i> Selanjutnya</a>
                    <?php endif ?>
                </div>
            </form>
            </div>
