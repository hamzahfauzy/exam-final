...
{
    if(!url)
        url = '<?= route('participant/exam-partial') ?>?token=<?=$_GET['token']?>'
    fetch(url)
    .then(res => res.text())
    .then(res => {
...
{
    let request = await fetch('<?= route('participant/exam/answer') ?>?token=<?=$_GET['token']?>',{
        method:'POST',
        headers : {
...
function finishExam()
{
    Swal.fire({
        ...
        if (result.value) {
            location='<?= route('participant/exam/finish') ?>?token=<?=$_GET['token']?>'
        }
    })
}
...
{
    fetch("<?= base_url() ?>/participant/load-navigation?token=<?=$_GET['token']?>&page="+page)
    .then(res => res.json())
    .then(res => {
        console.log(res)
        ...
        for(i=0;i<res.numOf;i++){
            ...
            <div class="col-sm-4" style="margin-bottom:10px">
                <a href="<?= base_url() ?>/participant/exam-partial/${number}?token=<?=$_GET['token']?>" onclick="nextQuestion(this)" data-page="${number}" class="btn btn-block ${answered}">${number}</a>
            </div>
            `
        }
    })
