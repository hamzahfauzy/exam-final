    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="post" id="editParticipantForm" action="<?= route('admin/question/insert') ?>">
            ...
            <!--    <input type="tel" name="status_login" id="status_login" class="form-control" required>-->
            <!--</div>-->
        </form>
      </div>
      <div class="modal-footer">
...
async function loadData(page = 1)
{
    ...
    $('.table-peserta > tbody').html('<tr><td colspan="6"><i>Loading...</i></td></tr>')
...
async function loadAllPeserta()
{
    let request = await fetch('<?= route('admin/participant/get-all') ?>')
    let response = await request.json()
    semuaPeserta = response
}
...
async function fetchEditPeserta(id)
{
    ...
    // $('#editParticipantForm').find('#status_login').val(response.login_status)
}
...
async function editPeserta()
{
    var data = {
        ...
        login_status:$('#editParticipantForm').find('#status_login').val(),
    }
...
function fetchToTable(data = false)
{
    ...
    if(data.length == 0)
    {
        $('.table-peserta > tbody').html('<tr><td colspan="6"><i>Tidak ada data!</i></td></tr>')
    }
    else
    {
    ...
    data.forEach(val => {
        $('.table-peserta > tbody').append(`<tr>
            ...
            <td>
                ${val.login_status ? 'Online' : 'Offline'}
            </td>
            <td>
...
function filterPeserta(keyword)
{
    ...
    if(keyword == "")
    {
        ...
        data = semuaPeserta.filter(peserta => {
