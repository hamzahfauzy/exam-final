  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <button class="btn btn-primary btn-notifikasi-email"><i class="fa fa-send"></i> Kirim Email Ke Semua Peserta</button>
        <p></p>
...
async function sendEmailNotification(id)
{
    Swal.fire({
        title: 'Konfirmasi ?',
        text: "Apakah anda yakin akan mengirim notifikasi email ke seluruh peserta ini ?",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        ...
        if (result.value) {
            let request = await fetch('<?= route('admin/kuis/notifikasi-email-peserta') ?>',{
                method :'POST',
                headers : {
                    'Content-Type':'application/json'
                },
...
async function fetchPesertaKuis(id)
{
    ...
    $('.btn-notifikasi-email').on('click', function(){
