...
class KuisController
{
    ...
    function notificationParticipantJson($id)
    {
        // $participants = User::where('notes','!=','NULL')->limit($id)->get();
        // $response = [];

        // foreach($participants as $participant)
        // {
        //     if(empty($participant->notes) || $participant->notes == NULL)
        //     {
        //         $chars = "0123456789";
        //         $password = substr(str_shuffle($chars),0,4);
        //         $participant->save([
        ...
        //     $response[] = ['no_hp' => $participant->meta('no_hp'), 'pesan' => $message];
        // }

        // return $response;
        $participants = $this->getParticipant($id);
        $response = [];
        ...
        foreach($participants as $participant)
        {
            ...
            $nama = "PT. Kawasan Industri Nusantara";
            $email = $participant->user_login;
            $link = "exam.tmcindonesia.com";
            $message = "Info ujian ".$nama.", Link: ".$link.", Email: ".$email.", Sandi: ".$password.", masuk untuk lihat jadwal";
    ...
    function participant($id)
    {
        ...
        $category = $this->getCategory($kuis->id);
        $jumlah_soal = 0;
        ...
        foreach($kuis->sesi() as $sesi)
        {
            foreach($sesi->partSession() as $partSesi)
            {
                ...
                foreach($jawaban as $jwb)
                    $skor += $jwb->status;
                $partSesi->skor = $skor > $jumlah_soal ? $jumlah_soal : $skor;
                $partSesi->jumlah_terjawab = count($jawaban) > $jumlah_soal ? $jumlah_soal : count($jawaban);
                $partSesi->sesi();
                $partSesi->origins = count($jawaban);
                $partSesi->jumlah_soal = $jumlah_soal;
