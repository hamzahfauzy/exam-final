...
class IndexController 
{
    ...
    function index()
    {
        ...
        $participant = Participant::find(session()->user()->id);
        // $participant = Participant::where('auth_token',$token)->first();
        $currentSession = 0;
        $nextSession = 0;
        foreach($participant->sesi() as $partSesi)
        {
            ...
            // if(($now > $waktu_mulai && $now < $waktu_selesai && !$currentSession) || (!$partSesi->partSesi() || $partSesi->partSesi->status == 1))
            if($now > $waktu_mulai && !$currentSession)
            {
    ...
    function exam($no = 1)
    {
        ...
        if(empty($sesi))
            return redirect(route('participant')."?token=".$_GET['token']);
        $partSesi = ParticipantSession::where('post_exam_id',$sesi->post_id)->where('user_id',session()->user()->id)->first();
        ...
        $s = $soal[$index];
        $participantAnswer = ParticipantAnswer::where('post_exam_id',$partSesi->sesi()->post_parent_id)->where('question_id',$s->id)->where('user_id',session()->user()->id)->first();
        ...
        $answer = ExamAnswer::where('exam_id',$partSesi->sesi()->post_parent_id)->where('question_id',$s->id)->where('user_id',session()->user()->id)->first();
    ...
    function loadNavigation()
    {
        ...
        $s = $soal;
        // $examQuestion = ExamQuestion::where('post_exam_id',$partSesi->sesi()->post_parent_id)->where('post_question_id',$s->id)->first();
        $answer = ExamAnswer::where('exam_id',$partSesi->sesi()->post_parent_id)->where('user_id',session()->user()->id)->get();
    ...
    function answer()
    {
        ...
        $examAnswer = ExamAnswer::where('exam_id',$exam->id)->where('question_id',$request->question_id)->where('user_id',session()->user()->id)->first();
        // $examAnswer = ExamAnswer::where('exam_question_id',$examQuestion->id)->where('user_id',session()->user()->id)->first();
        if(!$examAnswer)
    ...
    function finish()
    {
        ...
        return route('participant')."?token=".$_GET['token'];
    }
}